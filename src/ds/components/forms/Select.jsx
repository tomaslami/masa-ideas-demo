...
export function Select({ label, hint, error, required = false, options = [], placeholder, size = 'md', disabled = false, fullWidth = true, id, value, onChange, style = {}, containerStyle = {}, children, ...rest }) {
  ...
  return (
    <div style={{ width: fullWidth ? '100%' : undefined, ...containerStyle }}>
      ...
      <div style={{ position: 'relative', display: 'flex', alignItems: 'center' }}>
        ...
        <Icon name="chevron-down" size={16} style={{ position: 'absolute', right: s.px, pointerEvents: 'none', color: focus ? 'var(--accent-text)' : 'var(--text-muted)' }} />
      </div>
      {(hint || error) && (
