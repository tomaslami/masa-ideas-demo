...
export function Input({
  ...
  return (
    <div style={{ width: fullWidth ? '100%' : undefined, ...containerStyle }}>
      {label && (
        <label htmlFor={rid} style={{ display: 'block', fontSize: 'var(--text-sm)', fontWeight: 'var(--fw-medium)', color: 'var(--text-body)', marginBottom: 6 }}>
          {label}
        </label>
      )}
      <div
      ...
      >
        {icon && <Icon name={icon} size={16} style={{ color: 'var(--text-faint)' }} />}
