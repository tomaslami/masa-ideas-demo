const { Sidebar, Button, IconButton, Icon, ImportDialog } = window.DesignSystem_27db72;
const { Stepper, StepCliente, StepCarteles, StepCreatividad, StepCondiciones } = window.ProposalSteps;
  ...
  return (
    <div style={{ display: 'flex', width: '100vw', height: '100vh', overflow: 'hidden', background: 'var(--bg-page)' }}>
      <Sidebar logoSrc="../../assets/logo_mark.png" brand="Masa Ideas" product="Gestión interna"
        items={NAV} active="propuestas" onSelect={() => {}}
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
        ...
        <div style={{ flex: 1, overflow: 'auto', padding: step === 5 ? '28px 24px' : '32px 28px', background: step === 5 ? 'var(--bg-sunken)' : 'var(--bg-page)' }}>
          ...
          {step === 5 && (
            <div>
              <window.ProposalDoc cliente={cliente.nombre} meses={meses} desc={desc} selected={selected} fmt={fmt} creatives={creatives} />
              <div style={{ height: 28 }} />
            </div>
          )}
        </div>
